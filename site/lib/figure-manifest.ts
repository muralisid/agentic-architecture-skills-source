...
const foundationalFigures: FigureManifestEntry[] = [
  ...
  defineFigure({
    ...
    title: 'The intelligence ladder, and the test for leaving each rung',
    takeaway: 'Climb only when the rung below is measurably exhausted.',
    caption: 'Each rung changes something different. The gate on each is the evidence that justifies the next step.',
    alt: 'Seven rungs from instructions through context, tools and the loop, adapters, distillation, reinforcement fine-tuning, and pretraining, each with the exit test that justifies climbing.',
    ...
    data: { stages: [
      ...
      { label: 'Tools and the loop', detail: 'Change the system around the model, not the model.', gate: 'Loop instrumented, failures traced', kind: 'system' },
      { label: 'Adapters', detail: 'Train a small set of extra weights on your examples.', gate: 'Format fixed, judgement not', kind: 'agent' },
      { label: 'Distillation', detail: 'Same capability from a smaller, cheaper model.', gate: 'Cost or latency, not capability', kind: 'neutral' },
      { label: 'Reinforcement fine-tuning', detail: 'Train against a grader that scores the outcome.', gate: 'Grader cannot be gamed', kind: 'control' },
      { label: 'Pretraining and re-architecture', detail: 'Build or redesign the model itself.', gate: 'Essentially never, for an enterprise', kind: 'risk' },
    ] },
  }),
  defineFigure({
