...
const foundationalFigures: FigureManifestEntry[] = [
  ...
  defineFigure({
    id: 'intelligence-ladder', category: 'foundation', page: '/ladder', placement: 'The ladder', sourceHeading: 'Improve the agent system',
    ...
    caption: 'Each method changes something different. The gate on each is the evidence that justifies the next step.',
    alt: 'Three sequential steps, instructions then context then tools and the loop, each with the test that justifies moving to the next.',
    type: 'journey', evidenceStatus: 'mixed-evidence', dataBearing: false,
  ...
  defineFigure({
    id: 'model-change-diagnosis', category: 'foundation', page: '/ladder', placement: 'Model adaptation', sourceHeading: 'Adapt the model',
    title: 'Adapt the model to the task',
    takeaway: 'These are not steps up. Read what is failing, and expect to combine more than one.',
    caption: 'Supervised fine-tuning teaches behaviour from pairs; continued pretraining teaches the domain from raw text. Confusing the two is the common failure.',
    alt: 'Five options for changing the model, each matched to the symptom it addresses: behaviour, cost, judgement, domain knowledge, and data that is not text-shaped.',
    type: 'comparison', evidenceStatus: 'mixed-evidence', dataBearing: false,
    sources: [guideSource('Intelligence and learning', '/layers/r06-intelligence-and-learning'), guideSource('The decision catalog', '/decisions')],
    data: { columns: [
      { label: 'Wrong format or behaviour', kind: 'agent', items: ['Adapters and fine-tuning', 'Learns from thousands of pairs', 'Does not add knowledge', 'Causes forgetting'] },
      { label: 'Too slow or too costly', kind: 'neutral', items: ['Distillation', 'Same quality, smaller model', 'A move sideways, not up', 'Also repairs forgetting'] },
      { label: 'Wrong judgement', kind: 'control', items: ['Reinforcement fine-tuning', 'Needs an automatic grader', 'Outcomes must be checkable', 'Watch for reward hacking'] },
      { label: 'Does not know your domain', kind: 'evidence', items: ['Continued pretraining', 'Billions of raw tokens', 'The data-rich option', 'Pairs with fine-tuning after'] },
      { label: 'Data is not text-shaped', kind: 'risk', items: ['Custom pretraining', 'Graphs, sequences, signals', 'A different discipline', 'Rarely the answer'] },
    ] },
  }),
  defineFigure({
    id: 'prompt-anatomy', category: 'foundation', page: '/ladder/instructions', placement: 'What a prompt is made of', sourceHeading: 'What a prompt is actually made of',
    title: 'The five parts of a prompt, in cache order',
    takeaway: 'Stable parts first: everything before the first change is billed at cache rates.',
    caption: 'The system instruction, examples and definitions never change between calls, so they sit at the front and stay cached. The input varies, so it goes last.',
    alt: 'Five parts of a prompt in order: system instruction, task instruction, worked examples, the varying input, and the enforced output contract.',
    type: 'journey', evidenceStatus: 'conceptual-guide', dataBearing: false,
    sources: [guideSource('Step 1: Instructions', '/ladder/instructions')],
    data: { stages: [
      { label: 'System instruction', detail: 'Standing rules. Identical on every call.', gate: 'Cached', kind: 'control' },
      { label: 'Task instruction', detail: 'What this job is. Stable per workload.', gate: 'Cached', kind: 'system' },
      { label: 'Examples', detail: 'Two or three completed instances of the task.', gate: 'Cached', kind: 'evidence' },
      { label: 'Input', detail: 'The material for this instance. Varies every call.', gate: 'Billed fresh', kind: 'neutral' },
      { label: 'Output contract', detail: 'A schema the reply must match, enforced.', gate: 'Enforced, not requested', kind: 'agent' },
    ] },
  }),
  defineFigure({
    id: 'context-pipeline-ladder', category: 'foundation', page: '/ladder/context', placement: 'The pipeline', sourceHeading: 'The pipeline, in order',
    title: 'From a document to a retrieved answer',
    takeaway: 'Each stage sets the ceiling for the next; parsing errors are never recovered downstream.',
    caption: 'Parse keeps structure, chunk sets granularity, embed fixes the encoder version, index carries permissions, retrieve fuses meaning and keywords.',
    alt: 'Five pipeline stages from parse through chunk, embed and index to retrieve, each with the failure it guards against.',
    type: 'flow', evidenceStatus: 'conceptual-guide', dataBearing: false,
    sources: [guideSource('Step 2: Context', '/ladder/context'), guideSource('The data-to-memory pipeline', '/architecture/data-to-memory')],
    data: { stages: [
      { label: 'Parse', detail: 'Text out of the file, structure intact.', gate: 'Spot-checked forever', kind: 'system' },
      { label: 'Chunk', detail: 'Split into retrievable pieces, per corpus.', gate: 'Evaluated, not configured', kind: 'system' },
      { label: 'Embed', detail: 'Each piece becomes a vector, versioned.', gate: 'Encoder version pinned', kind: 'evidence' },
      { label: 'Index', detail: 'Stored with the source permissions inside.', gate: 'Filter before search', kind: 'control' },
      { label: 'Retrieve', detail: 'Meaning and keyword search, fused.', gate: 'Beats a matched control', kind: 'agent' },
    ] },
  }),
  defineFigure({
    id: 'agent-loop-anatomy', category: 'foundation', page: '/ladder/tools-and-the-loop', placement: 'How the loop runs', sourceHeading: 'How the loop actually runs',
    title: 'One turn of an agent loop',
    takeaway: 'The model is stateless: every turn resends everything, and nothing stops the loop unless the harness stops it.',
    caption: 'The harness owns every box except the first. Caps on iterations, wall clock and cost sit between the result and the next turn.',
    alt: 'The agent loop as a cycle: the model decides, a tool executes, the result is appended, the harness checks its caps, and the loop repeats or stops.',
    type: 'loop', evidenceStatus: 'conceptual-guide', dataBearing: false,
    sources: [guideSource('Step 3: Tools and the loop', '/ladder/tools-and-the-loop'), guideSource('The agent platform layer', '/layers/r07-agent-platform')],
    data: { stages: [
      { label: 'Model decides', detail: 'Answer, or ask for a tool.', kind: 'agent' },
      { label: 'Gateway checks', detail: 'May this caller do this, now?', kind: 'control' },
      { label: 'Tool executes', detail: 'Your code runs; the model waits.', kind: 'system' },
      { label: 'Result appended', detail: 'Context grows every turn.', kind: 'evidence' },
      { label: 'Caps checked', detail: 'Iterations, wall clock, cost.', kind: 'control' },
      { label: 'Repeat or stop', detail: 'Reset context; keep state in records.', kind: 'human' },
    ] },
  }),
  defineFigure({
    id: 'tuning-methods', category: 'foundation', page: '/ladder/adapters-and-fine-tuning', placement: 'The methods', sourceHeading: 'Full fine-tuning, LoRA, and QLoRA',
    title: 'Three ways to train on examples',
    takeaway: 'Same mechanism, different footprint. None of the three stores facts, and none prevents forgetting.',
    caption: 'LoRA approximates the change with two small matrices; QLoRA quantises the frozen base so one serious GPU is enough.',
    alt: 'Comparison of full fine-tuning, LoRA and QLoRA across what trains, the artifact produced, and the hardware needed.',
    type: 'comparison', evidenceStatus: 'conceptual-guide', dataBearing: false,
    sources: [guideSource('Adapters and supervised fine-tuning', '/ladder/adapters-and-fine-tuning')],
    data: { columns: [
      { label: 'Full fine-tuning', kind: 'risk', items: ['Every weight updates', 'Artifact: a whole model', 'Multi-GPU training', 'Maximum drift, maximum cost'] },
      { label: 'LoRA', kind: 'agent', items: ['Two small matrices per layer', 'Artifact: megabyte adapter', 'Base model stays frozen', 'Swap adapters per task'] },
      { label: 'QLoRA', kind: 'evidence', items: ['LoRA over a 4-bit base', 'Artifact: megabyte adapter', 'One serious GPU', 'Near-LoRA quality, less memory'] },
    ] },
  }),
  defineFigure({
    id: 'distillation-two-uses', category: 'foundation', page: '/ladder/distillation', placement: 'The two uses', sourceHeading: 'The second use: repairing forgetting',
    title: 'One mechanism, two jobs',
    takeaway: 'The student learns the teacher\u2019s distribution, not just its answers. Who the teacher is decides which job you are doing.',
    caption: 'The volume play distils a proven capability into a cheaper model. The repair play uses the pre-fine-tune model as teacher to restore what domain training damaged.',
    alt: 'Two uses of distillation compared: compressing a proven capability into a smaller model, and repairing forgetting using the pre-fine-tune model as teacher.',
    type: 'comparison', evidenceStatus: 'mixed-evidence', dataBearing: false,
    sources: [guideSource('Distillation', '/ladder/distillation'), guideSource('Intelligence and learning', '/layers/r06-intelligence-and-learning')],
    data: { columns: [
      { label: 'The volume play', kind: 'neutral', items: ['Teacher: the big proven model', 'Student: small, task-scoped', 'Buys unit cost and latency', 'Justified by call volume'] },
      { label: 'The repair play', kind: 'evidence', items: ['Teacher: the pre-fine-tune model', 'Student: your fine-tuned model', 'Restores general capability', 'Run after chapters 4 or 7'] },
    ] },
  }),
  defineFigure({
    id: 'grader-gameability', category: 'foundation', page: '/ladder/reinforcement-fine-tuning', placement: 'The entry requirement', sourceHeading: 'What makes a grader gameable',
    title: 'Where grading is safe, and where it will be gamed',
    takeaway: 'The model becomes excellent at whatever the grader actually measures. Score and competence must be inseparable.',
    caption: 'A score above what careful people achieve on the same task means the rubric was learned, not the job. Treat it as a defect report.',
    alt: 'Comparison of tasks safe to grade programmatically against grader designs that will be gamed, with the human-ceiling alarm.',
    type: 'comparison', evidenceStatus: 'mixed-evidence', dataBearing: false,
    sources: [guideSource('Reinforcement fine-tuning', '/ladder/reinforcement-fine-tuning'), guideSource('The learning flywheel', '/architecture/learning-flywheel')],
    data: { columns: [
      { label: 'Safe to grade', kind: 'evidence', items: ['Code compiles, tests pass', 'Figure matches the filing', 'Every claim traces to a source', 'Checkable by program'] },
      { label: 'Will be gamed', kind: 'risk', items: ['Style and confidence proxies', 'Length as effort', 'An LLM judging its own family', 'Three checks, the rest neglected'] },
      { label: 'The alarm', kind: 'control', items: ['Humans agree 92 to 95 percent', 'Model scores 99 percent', 'The checker has a hole', 'File it as an incident'] },
    ] },
  }),
  defineFigure({
    id: 'sft-vs-cpt', category: 'foundation', page: '/ladder/continued-pretraining', placement: 'The distinction', sourceHeading: 'The distinction, one more time',
    title: 'Teaching behaviour, teaching the domain',
    takeaway: 'Different data, different volume, different job. Most enterprises have the raw text and few labelled pairs.',
    caption: 'On a fixed budget, the published optimum put roughly 99.99 percent of tokens into continued pretraining and the small remainder into fine-tuning on top.',
    alt: 'Supervised fine-tuning and continued pretraining compared: pairs versus raw text, thousands versus billions of tokens, behaviour versus knowledge.',
    type: 'comparison', evidenceStatus: 'preprint-or-prototype', dataBearing: false,
    sources: [guideSource('Continued pretraining', '/ladder/continued-pretraining')],
    data: { columns: [
      { label: 'Supervised fine-tuning', kind: 'agent', items: ['Input: question and answer pairs', 'Thousands of examples', 'Teaches how to behave', 'Someone must write the answers'] },
      { label: 'Continued pretraining', kind: 'evidence', items: ['Input: raw text as it exists', 'Billions of tokens', 'Teaches what the domain is', 'The corpus must be curated'] },
      { label: 'Together', kind: 'control', items: ['Pretrain first, then fine-tune', 'Domain first, task second', 'Most budget to pretraining', 'Repair forgetting after'] },
    ] },
  }),
  defineFigure({
    id: 'top-of-market-paths', category: 'foundation', page: '/ladder/custom-pretraining', placement: 'The two paths', sourceHeading: 'Path one: a foundation model of your own',
    title: 'Two different things sold in similar words',
    takeaway: 'One is an economics question and almost never the answer. The other is a real discipline for data that is not text.',
    caption: 'Before either: what would continued pretraining on the same corpus achieve, and does the exotic option beat the boring ones on your own labels?',
    alt: 'Two paths at the top of the market compared: training a foundation model from scratch, and designing a graph model for network-shaped data.',
    type: 'comparison', evidenceStatus: 'mixed-evidence', dataBearing: false,
    sources: [guideSource('Custom pretraining and architecture', '/ladder/custom-pretraining')],
    data: { columns: [
      { label: 'Your own foundation model', kind: 'risk', items: ['Pretrain from random weights', 'Frontier-lab economics', 'Perishable against the field', 'Genuine only under sovereignty mandates'] },
      { label: 'A different architecture', kind: 'system', items: ['Data is a network, not a document', 'Graph model, two or three layers deep', 'Over-smoothing versus over-squashing', 'Meets the agent at a tool boundary'] },
    ] },
  }),
  defineFigure({
