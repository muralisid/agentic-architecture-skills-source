...
export const GLOSSARY: GlossaryEntry[] = [
  ...
  // Retrieval and the research pages
  { key: 'pooled embedding', term: 'Pooled (single-vector) embedding', group: 'Data and knowledge', aliases: ['pooled', 'single vector', 'single-vector', 'one vector'], plain: 'One numeric fingerprint for a whole document. Cheap and standard, but for a document about several things it is an average of all of them, so it matches each of them weakly.' },
  { key: 'multi-view embeddings', term: 'Multi-view (multi-card) embeddings', group: 'Data and knowledge', aliases: ['multi-view', 'multi-card', 'cards', 'card', 'purpose views', 'views', 'purpose-specific views'], plain: 'Giving one document several numeric fingerprints instead of one, each built from the text about one purpose (what is asked, what was promised, what went wrong), so a question about one aspect matches that aspect rather than a blurred average.' },
  { key: 'matched control', term: 'Matched control', group: 'Learning and quality', aliases: ['matched chunks', 'matched-capacity', 'matched capacity'], plain: 'A comparison arm given the same budget as the method under test, here the same number of embeddings per document, so the experiment measures the idea rather than the extra capacity.' },
  { key: 'bm25', term: 'BM25 (keyword search)', group: 'Data and knowledge', aliases: ['lexical search', 'keyword search', 'lexical'], plain: 'The classic keyword ranking formula behind most search engines. It matches exact words, so it wins on names, codes, and quotations, where meaning-based search is weak.' },
  { key: 'hybrid retrieval', term: 'Hybrid retrieval (reciprocal rank fusion)', group: 'Data and knowledge', aliases: ['rrf', 'reciprocal rank fusion', 'hybrid', 'fusion'], plain: 'Running keyword search and meaning-based search side by side and merging the two ranked lists by rank position, so each covers the other\'s blind spots.' },
  { key: 'topic modelling', term: 'Topic modelling', group: 'Data and knowledge', aliases: ['topic model', 'topic discovery', 'clustering', 'density clustering'], plain: 'Grouping documents into themes automatically by clustering their embeddings, then naming each group. Here a model names and merges the groups from keyword lists; it never reads every document.' },
  { key: 'relevance gate', term: 'Relevance gate', group: 'Data and knowledge', aliases: ['gate', 'cheap gate', 'pre-filter', 'first pass'], plain: 'A cheap first pass that scores every document against a few anchor phrases in embedding space and sets aside the off-purpose ones before any expensive processing. Discarded items are quarantined, not deleted.' },
  { key: 'instruction-following retriever', term: 'Instruction-following retriever', group: 'Data and knowledge', aliases: ['instruction-conditioned encoder', 'instruction-tuned retriever', 'instruction-conditioned', 'instruction-prepended'], plain: 'An embedding model trained so that a stated instruction (for example, only reports from outside the UK) changes what it ranks as relevant, instead of only the query words.' },
  { key: 'diversity selection', term: 'Diversity selection (MMR, DPP, harvest)', group: 'Data and knowledge', aliases: ['mmr', 'dpp', 'outlier harvest', 'harvest', 'diversification'], plain: 'Rules for choosing which ten candidates to pass on so that they cover different sub-topics rather than repeating the top hit. MMR and DPP re-rank the whole list; the harvest keeps the most relevant head and spends a fixed quota on the least similar tail.' },
  { key: 'ndcg', term: 'nDCG@10', group: 'Learning and quality', aliases: ['ndcg@10', 'ndcg at 10', 'normalised discounted cumulative gain'], plain: 'A 0 to 1 score of how good the first ten search results are. It rewards putting the right documents near the top and counts every relevant document the judges marked. A difference of 0.03 is small but real on a few hundred queries; 0.2 is large.' },
  { key: 'coverage metrics', term: 'Coverage metrics (S-recall, alpha-nDCG)', group: 'Learning and quality', aliases: ['s-recall', 'subtopic recall', 'alpha-ndcg', 'coverage'], plain: 'Scores that reward a result list for covering different sub-topics rather than repeating the same one. S-recall is the share of known sub-topics that appear in the top ten; alpha-nDCG discounts repeats.' },
  { key: 'paired permutation test', term: 'Paired permutation test (p-value)', group: 'Learning and quality', aliases: ['p-value', 'p value', 'holm', 'holm correction', 'permutation test', 'confidence interval', 'bootstrap'], plain: 'A statistical check that compares two systems query by query and asks how often a difference this large would appear by chance. A p-value of 0.0001 means almost never; 0.35 means the difference is within noise. Holm correction raises the bar when many comparisons run at once.' },
  { key: 'p-mrr', term: 'p-MRR (instruction following score)', group: 'Learning and quality', aliases: ['instruction following score'], plain: 'A FollowIR benchmark score of whether a search system changes its ranking the right way when the instruction changes. Zero means the system ignores the instruction.' },
  { key: 'aspect', term: 'Aspect (of a document)', group: 'Data and knowledge', aliases: ['aspects', 'aspect-rich', 'multi-aspect'], plain: 'One of the distinct things a document is about. An email that confirms a delivery date, flags a billing error, and asks for a contract change has three aspects; a question usually targets one of them.' },
