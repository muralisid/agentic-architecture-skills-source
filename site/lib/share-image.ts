 ...
 *
 * The lookup is by convention, public/figures/<slug>.png against the page's
 * own slug, so adding a figure to a page gives it a share image with no second
 * place to register it and nothing to keep in step.
 *
 ...
 * filed as patterns/index.png, so both spellings are tried.
 */
export function getProvidedFigure(slugs: string[]): string | null {
  if (slugs.length === 0) return null;
  const direct = slugs.join('/');
  const asIndex = `${direct}/index`;
  if (figures.has(direct)) return `/figures/${direct}.png`;
  if (figures.has(asIndex)) return `/figures/${asIndex}.png`;
  return null;
}
