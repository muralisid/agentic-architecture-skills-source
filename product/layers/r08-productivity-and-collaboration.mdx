...
<TeachingIllustration src="/images/layers/r08-identity-presence-labeled-v1.webp" alt="Every agent receives a service principal, named sponsor, scoped connectors and automatic revocation; only exceptional agents receive a separately approved mailbox, meeting seat or user-like presence." title="Identity is universal; human-like presence is a separate grant" caption="Every agent needs an auditable directory identity, but few need to appear as a user. Mailboxes, meeting seats and other presence surfaces add impersonation and consent risk, so they require explicit justification." />
...
<TeachingIllustration src="/images/layers/r08-task-team-labeled-v1.webp" alt="Assistants show measurable gains on solitary tasks such as drafting, summarising and research, while meetings, decisions and handoffs improve only when team norms change." title="Deploy assistants by task, not by licence availability" caption="Individual acceleration does not automatically change coordinated work. Measure each task's outcome, assume review will be shallow, and retain only the workflows that improve real quality or time." />
