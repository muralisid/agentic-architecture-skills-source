...
<TeachingIllustration src="/images/layers/r01-safe-session-labeled-v1.webp" alt="A task enters through a dedicated workload identity and isolated microVM session, reaches approved destinations only through an egress proxy, and records durable state for pause, crash and resume." title="The safe session is the unit of agent infrastructure" caption="Isolation, identity, egress control, durable state and a kill switch form one runtime boundary. If any one is missing, a capable agent can escape the assumptions made about the others." />
...
<TeachingIllustration src="/images/layers/r01-model-location-labeled-v1.webp" alt="Three model-serving lanes route lower-sensitivity work to metered APIs, restricted work to sovereign regions, and the highest-sensitivity work to air-gapped or self-hosted infrastructure." title="Sensitivity chooses the location; utilisation chooses the economics" caption="Managed metered access is the default. Sovereign or air-gapped serving is earned by classification, while owning model capacity is justified only by sustained utilisation or a control requirement." />
