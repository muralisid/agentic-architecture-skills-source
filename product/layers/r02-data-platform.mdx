...
<TeachingIllustration src="/images/layers/r02-permission-retrieval-labeled-v1.webp" alt="Caller identity filters unauthorised documents before a governed index returns source spans and a cited answer, while high-stakes numbers pass through a semantic contract." title="Permission is applied before retrieval, not after generation" caption="The caller's live identity narrows the searchable set before similarity ranking happens. High-stakes numeric questions take a separate deterministic path that produces a governed answer or an explicit refusal." />
...
<TeachingIllustration src="/images/layers/r02-fragment-rules-labeled-v1.webp" alt="A searchable data chunk carries access rules, source provenance, residency, embedding version and freshness metadata; its vector and answer inherit the strictest source rule." title="Every derived fragment must keep its governance envelope" caption="A chunk is not just text. Its permissions, provenance, residency, version and freshness must survive parsing, embedding, indexing and answer generation, because losing that metadata loses the ability to govern the result." />
