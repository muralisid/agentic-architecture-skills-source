...
<TeachingIllustration src="/figures/memory/time-series.svg" mobileSrc="/figures/memory/time-series-mobile.svg" title="Represent how equipment behaves over time" alt="Three illustrative signals compare the current pump with a similar shape at a different magnitude and a similar peak lasting longer. The charts are independently scaled; similarity does not prove the same cause." caption="Decide whether shape, magnitude, or duration matters, then retain the original readings to check the match." />
